...
def get_last_md5_file(resource_dir, resource_type, resource_regexp, target_suffix, suffix):
    ...
    for current_file in files:
        if match_obj := re.search(
            rf'{resource_regexp}{target_suffix}{suffix}.md5$', current_file
        ):
    ...
    with open(current_resource, 'r') as current_file:
        if match_obj := re.search(r'^(\w+)\s', current_file.read()):
            return match_obj[1]
    return 'f'
...
def download_file_from_server_endpoint(server_endpoint, local_file_path):
    ...
    if response.status_code == 200:
        ...
        try:
            log('INFO', f'Downloading file as {local_file_path}')
            with open(local_file_path, 'wb') as local_file:
                for chunk in response.iter_content(chunk_size=128):
                    local_file.write(chunk)
                    # log('DEBUG', 'Downloaded file as {}'.format(local_file_path))
        except Exception:
            log('WARNING', f'Unable to download {server_endpoint}')
    else:
        log('WARNING', f'Unable to contact {server_endpoint}')
...
def get_new_ncbi_resource_file(http, resource_type, resource_dir, regexp, label, url, target_suffix):
    ...
    if match_obj:
        ...
        try:
            ...
            if match_obj := re.search(r'^(\w+)\s', resource_md5):
                distant_md5 = match_obj[1]
...
def main():
    ...
    if args.clingen_criteria:
        ...
        clingen_new_file = f'clingenCriteriaSpec_{date_string}'
        ...
        current_md5_value = get_last_md5_file(
            f'{resources_path}clingen/',
            'clingenCriteriaSpec',
            r'clingenCriteriaSpec_(\d+)',
            '',
            '',
        )
        BLOCKSIZE = 65536
        ...
        if hasher.hexdigest() != current_md5_value:
            ...
            with open('{0}clingen/{1}.md5'.format(resources_path, clingen_new_file), "w") as new_md5:
            ...
            clingen_file.close()
            with open('{0}clingen/{1}.txt'.format(resources_path, clingen_new_file), "w") as genes_symbols_file:
    ...
    if args.dbsnp:
        ...
        download_file_from_server_endpoint(
            f'{dbsnp_url}release_notes.txt',
            f'{resources_path}dbsnp/release_notes.txt',
        )
        with open(f'{resources_path}dbsnp/release_notes.txt', 'r') as f:
            match_obj = re.search(r'dbSNP build (\d+) release notes', f.readline())
            semaph = 0
            if match_obj:
                dbsnp_version = match_obj[1]
                ...
                log(
                    'ERROR',
                    f'Unable to donwload/read dbSNP release file from {dbsnp_url}release_notes.txt',
                )
            if semaph == 0:
                # we can proceed
