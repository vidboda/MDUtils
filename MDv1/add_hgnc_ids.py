...
def get_aliases(previous_symbols, current_aliases):
    ...
    if aliases_obj := re.search(r'^;(.+)$', aliases):
        aliases = aliases_obj[1]
    if aliases_obj := re.search(r'^(.+);$', aliases):
        aliases = aliases_obj[1]
    return aliases
...
def main():
    ...
    for line in open(hgnc_file):
        ...
        hgnc_chr = None
        hgnc_id = None
        if hgnc[header['Status']] in ['Symbol Withdrawn', 'Entry Withdrawn']:
            continue
        if hgnc_chr_obj := re.search(
            r'^([\dXY]{1,2})[pq]*.*', hgnc[header['Chromosome']]
        ):
            hgnc_chr = hgnc_chr_obj[1]
        else:
            log('WARNING', 'Cannot identify Chr: line {0}: {1}'.format(i, line))
        if hgnc_id_obj := re.search(r'^HGNC:(\d+)$', hgnc[header['HGNC ID']]):
            hgnc_id = hgnc_id_obj[1]
        if hgnc_id:
            curs.execute(
            ...
            if md_gene := curs.fetchone():
                ...
                if md_gene['name'][0] == hgnc[header['Approved symbol']] and \
                    ...
                    if hgnc_chr and \
                        ...
                        if aliases not in [';', md_gene['second_name']]:
                            curs.execute(
                ...
                for name in previous_names_list:
                    ...
                    if md_gene_to_update := curs.fetchone():
                        # update_semaph = 1
            ...
            log('WARNING', f"Cannot identify HGNC ID: {hgnc[header['HGNC ID']]}")
